...
class Job(BaseModel):
    ...
    job_profile: Optional[StrictStr] = Field(default=None, description="Define what type of job is to be executed on the media object.", alias="jobProfile")
    region_name: Optional[StrictStr] = Field(default=None, description="If defined, require job to be run on a Worker instance that can serve any of the regions, else can run on any Worker instance. If no service is available for the requested regions, the job fails.", alias="regionName")
    max_region_latency: Optional[StrictInt] = Field(default=100, description="If defined, require job to be run on a Worker instance that can access the requested region with a latency less than this value", alias="maxRegionLatency")
    job_input: Optional[JobInput] = Field(default=None, alias="jobInput")
    __properties: ClassVar[List[str]] = ["jobType", "jobProfile", "regionName", "maxRegionLatency", "jobInput"]
    ...
    def job_type_validate_enum(cls, value):
        ...
        if value not in set(['AIJob', 'MediaProcessingJob', 'FaceRecognitionJob']):
            raise ValueError("must be one of enum values ('AIJob', 'MediaProcessingJob', 'FaceRecognitionJob')")
        return value

    @field_validator('job_profile')
    def job_profile_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['MediaCortexVideoAnalysis', 'MediaCortexSpeechToText', 'ExtractFaces', 'ClusterFaces', 'SearchFaces']):
            raise ValueError("must be one of enum values ('MediaCortexVideoAnalysis', 'MediaCortexSpeechToText', 'ExtractFaces', 'ClusterFaces', 'SearchFaces')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of job_input
        if self.job_input:
            _dict['jobInput'] = self.job_input.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "jobProfile": obj.get("jobProfile"),
            "regionName": obj.get("regionName"),
            "maxRegionLatency": obj.get("maxRegionLatency") if obj.get("maxRegionLatency") is not None else 100,
            "jobInput": JobInput.from_dict(obj["jobInput"]) if obj.get("jobInput") is not None else None
        })
        return _obj
