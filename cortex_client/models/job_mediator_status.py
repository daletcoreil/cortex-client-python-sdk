...
class JobMediatorStatus(BaseModel):
    """
    ...
    """ # noqa: E501
    status: StrictStr = Field(description="Current status of the job as reported by the Job Processor that is handling it, or FAILED if it was rejected (e.g., for missing quota). <br/>Possible transitions - NEW -> QUEUED -> RUNNING -> COMPLETED.")
    status_message: Optional[StrictStr] = Field(default=None, description="Last message reported by Job Processor handling this job.", alias="statusMessage")
    progress: Optional[StrictInt] = Field(default=None, description="Progress as reported by the Job Processor. (TBD)")
    __properties: ClassVar[List[str]] = ["status", "statusMessage", "progress"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NEW', 'FAILED', 'QUEUED', 'SCHEDULED', 'RUNNING', 'COMPLETED']):
            raise ValueError("must be one of enum values ('NEW', 'FAILED', 'QUEUED', 'SCHEDULED', 'RUNNING', 'COMPLETED')")
        return value
