...
class MediatorJob(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Job ID computed by the Dalet Media Mediator Job registry.  Uniquely identifies the job after it has been accepted by the mediator.")
    date_created: StrictStr = Field(description="Datetime when the job was accepted by the Mediator", alias="dateCreated")
    date_modified: StrictStr = Field(description="Datetime when the job was last updated by a Job Processor service reporting progress on this job.", alias="dateModified")
    quantity: StrictInt = Field(description="Verified number of units after the job is completed.  Units depend on the job profile (usually, they correspond to the duration of the media).  Invoicing is based on this value, which may be different from the one submitted by the client.")
    status: JobMediatorStatus
    job_output: Optional[JobOutput] = Field(default=None, alias="jobOutput")
    __properties: ClassVar[List[str]] = ["id", "dateCreated", "dateModified", "quantity", "status", "jobOutput"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of job_output
        if self.job_output:
            _dict['jobOutput'] = self.job_output.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "status": JobMediatorStatus.from_dict(obj["status"]) if obj.get("status") is not None else None,
            "jobOutput": JobOutput.from_dict(obj["jobOutput"]) if obj.get("jobOutput") is not None else None
        })
        return _obj
